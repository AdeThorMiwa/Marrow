...
// estimateStaleAfter derives how long this source can go without a new item
// before it's genuinely stale, from the average gap between its own recent
// dated items (up to staleAfterSampleSize) — a source's own natural posting
// cadence, not a global guess (a weekly channel going quiet for a week is
// normal; a daily one isn't). Averaging over several items rather than just
// the two most recent avoids a source that occasionally posts a burst (e.g.
// two items an hour apart, then nothing for a week) looking stale almost
// immediately. Falls back to defaultStaleAfter when there are fewer than
// two dated items to compare (a brand new or sparse feed).
