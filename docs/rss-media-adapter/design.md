...
- **Large-file memory behavior remains unsolved**, per the requirements doc's explicit Out of Scope — a full-size FLOSS/TWiT episode (~1-3GB) loaded entirely into `Media.Buffer` is a known real limitation, not a gap introduced by this design. Confirmed in practice, not just theoretical: real FLOSS episodes were observed timing out mid-download against the original 5-minute `http.Client` timeout in `RSSMediaResolver` — bumped to 60 minutes as a stopgap (`rss_media.go`), which doesn't touch the underlying full-in-memory-buffer approach.
  - **Possible future optimization: partial download via HTTP Range.** Rather than always fetching the whole file, a bounded prefix (or a specific byte range) could be enough for a useful transcript, trading completeness for speed — genuinely useful for large video episodes. Not as simple as "download the first/last N bytes" though: MP4 stores its metadata (`moov` atom, sample-to-byte-offset tables) either at the front or the back of the file depending on the encoder, and the actual audio/video samples (`mdat`) are a separate region — a naive tail fetch gets you metadata but no decodable audio. Doing this properly needs two passes: (1) a small Range request for the `moov` atom (front or back), parsed to find where the samples for the desired duration live in `mdat`, then (2) a second targeted Range request for just that byte span. Requires real MP4 box-parsing, not implemented — deliberately deferred (2026-08-05) in favor of the timeout bump above, since it trades transcript completeness for speed in a way that affects real embedding quality, not just testing convenience.
