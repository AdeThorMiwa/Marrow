...
import { type FeedFilter, getFeed } from "@/lib/feed";
import {
	addSourceToGroup,
	createGroup,
	listGroups,
...
function feedItemKeyOf(item: FeedItem): string {
	if (item.type === "source_health")
		return `source_health:${item.payload.source_id}`;
...
export default function HomeScreen() {
	...
	const [selection, setSelection] =
		useState<FilterSelection>(DEFAULT_SELECTION);
	...
	const loadFirstPage = useCallback(async () => {
		...
		try {
			const page = await getFeed(
				undefined,
				undefined,
				toFeedFilter(selectionRef.current),
			);
			setItems(page.items);
			setNewItems([]);
		...
		const interval = setInterval(async () => {
			try {
				...
				const fresh = page.items.filter(
					(item) => !knownKeysRef.current.has(feedItemKeyOf(item)),
				);
				...
				if (itemsLengthRef.current === 0) {
					setItems(fresh);
					fresh.forEach((item) =>
						knownKeysRef.current.add(feedItemKeyOf(item)),
					);
					setCursor(page.next_cursor || undefined);
					return;
				}
				...
				setNewItems((prev) => {
					...
					const merged = [
						...fresh.filter((item) => !seen.has(feedItemKeyOf(item))),
						...prev,
					];
					merged.forEach((item) =>
						knownKeysRef.current.add(feedItemKeyOf(item)),
				...
				>
					...
					<SourceRail
						...
						onSourcePausedChanged={(id, paused) =>
							setSources((prev) =>
								prev.map((s) => (s.id === id ? { ...s, paused } : s)),
							)
						}
						selection={selection}
						onToggle={(kind, id) =>
							setSelection((prev) => toggleFilterItem(prev, kind, id))
						}
						...
						<FlatList
							...
							renderItem={({ item }) => (
								<FeedRow
									item={item}
									horizontalInset={horizontalInset}
									isVisible={visibleKeys.has(feedItemKeyOf(item))}
								/>
							)}
						/>
					</View>
...
function ContentRow({
	...
	const title = payload.title ? (
		<Text variant="itemTitle">{payload.title}</Text>
	) : null;
	const media = (
		<ContentMedia type={type} blocks={payload.blocks} isVisible={isVisible} />
	);
	const summary = payload.summary ? (
		<Markdown size="small">{payload.summary}</Markdown>
	) : null;
...
function ContentMedia({
	...
	if (type === "video") {
		...
		if (youtubeVideoId)
			return <YouTubeEmbed videoId={youtubeVideoId} isVisible={isVisible} />;
	...
	if (imageUri) {
		return (
			<Image
				source={{ uri: imageUri, headers: getMediaHeaders() }}
				style={{
					width: "100%",
					height: 220,
					borderRadius: 16,
...
function SourceRail({
	...
	const [groupPickerSource, setGroupPickerSource] = useState<Source | null>(
		null,
	);
	...
	const handleTogglePauseGroup = useCallback(async () => {
		...
		try {
			...
			setGroups((prev) =>
				prev.map((g) => (g.id === id ? { ...g, paused: !paused } : g)),
			);
		} catch {
			// Silent-fail, same tolerance as the rest of this rail's mutations.
		}
	...
	const railData = useMemo(
		() => [
			...
			...groups
				...
				.map((g) => ({ kind: "group" as const, key: `g-${g.id}`, group: g })),
			...sources.map((s) => ({
		...
		<>
					...
					>
						...
						>
							<MaterialCommunityIcons
								name="plus"
								size={SOURCE_RAIL_LOGO_SIZE * 0.5}
			...
			<ActionSheet
				...
				actions={[
					{
						label: "+ New group",
						onPress: () => setCreatingGroupFor(groupPickerSource),
					},
					...groups
						.filter((g) => !g.is_default)
						.map((g) => ({
